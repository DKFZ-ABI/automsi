...
class CustomLoss():
    ...
    def supervised_error(x, x_pred, y, threshold):
        ...
        zero_mask =  tf.math.equal(tf.reduce_mean(x, axis=3), 0.0)
        non_zero_mask =  tf.logical_not(zero_mask)
        
        # retains y.shape, e.g., (batch_size, 3, 3)
        y_mask = tf.math.greater(y, threshold) 
        # only consider non-empty pixel
        y_mask = tf.logical_and(non_zero_mask, y_mask)
...
class ConvVAE(VAE):
    ...
    def __init__ (self, n_features : int, latent_dim : int,
                 patch_size : int, enc_conv_params : list, dec_conv_params : list, activation : str):
    ...
    def build(self, plot_vae = False):
        ...
        # decoder
        last_conv_layer = len(self.enc_conv_params) - 1
        latent_inputs = tf.keras.Input(shape=(self.latent_dim,), name = 'z_sampling')
        
        hdec = layers.Dense(int(self.patch_size-1) * int(self.patch_size-1) * 
                            self.enc_conv_params[last_conv_layer].filters)(latent_inputs)
        hdec = layers.BatchNormalization()(hdec) 
        hdec = layers.ReLU()(hdec) 
        hdec = layers.Reshape((int(self.patch_size-1), int(self.patch_size-1), self.enc_conv_params[last_conv_layer].filters))(hdec) ####
    ...
    def build_conv2dtranspose_layers_prev(self, prev_param, filters, inputs):
        hdec = layers.Conv2DTranspose(filters, (prev_param.kernel, prev_param.kernel), strides= prev_param.strides, padding="valid")(inputs)
        hdec = layers.BatchNormalization()(hdec)
        hdec = layers.ReLU()(hdec) 
        return(hdec)
...
class VAETrainer(tf.keras.Model, CustomLoss):
    def __init__(self, encoder, decoder, kl_coef, **kwargs):
    ...
    def do_step(self, data):
        ...
        reconstruction_loss = self.reconstruction_error(tf.cast(data, tf.float32), data_pred, 1)
...
class AdjConvParams(ConvParams):
    
    
    @staticmethod
    def build_power2_conv_layers_1x1_middle(conv_filter_max, conv_filter_min, conv_filter_step, kernel, stride, indices_for_3x1):
        enc_filters = []
        dec_filters = []
        dim = conv_filter_max
        i = 0
...
class Experiment():
    ...
    def split_x_y(self, patches):
        self.x, self.y = patches.x_mean, patches.y_mean
        return self
...
class ConvAEExperimentDeprecated(Experiment):
    def __init__(self, experiment, scaler = None, suffix = ""):
        ...
        self.conv_filter_1x1_pos = 0
        self.samples_no = "_".join(params[11:14+1])
        self.mode = params[15]
            

        self.enc_conv_params, self.dec_conv_params = ConvParams.build_power2_3x1_conv_layers(self.conv_filter_max, self.conv_filter_min, self.conv_filter_step, self.kernel, self.stride, [self.conv_filter_1x1_pos])
    ...
    def split_x_y(self, patches):
        self.x, self.y = patches.x, patches.y_mean
        return self
...
class ConvAEExperiment(ConvAEExperimentDeprecated):
    
    def __init__(self, experiment, scaler = None, suffix = ""):
        super().__init__(experiment, suffix, scaler)
        params = experiment.split("_")
        self.activation = params[4]
        self.kernel = int(params[6])
        self.stride = int(params[7])
        self.conv_filter_max = int(params[8])
        self.conv_filter_min = int(params[9])
        self.conv_filter_step = int(params[10])
        self.conv_filter_1x1_pos = int(params[11])
        self.samples_no = "_".join(params[13:16+1])
        self.mode = params[16]

        self.enc_conv_params, self.dec_conv_params = AdjConvParams.build_power2_conv_layers_1x1_middle(self.conv_filter_max, self.conv_filter_min, self.conv_filter_step, self.kernel, self.stride, [self.conv_filter_1x1_pos])
        
        
class ConvVAEExperiment(ConvAEExperiment):
    ...
    def build(self, n_features, path, experiment_with_suffix):
        conv_ae = ConvVAE(n_features, self.conv_filter_min, self.patch_size, self.enc_conv_params, self.dec_conv_params, self.activation).build(False)
        conv_ae.encoder.load_weights(path + "weights/" + experiment_with_suffix + '_encoder_model_weights.h5') 
        conv_ae.decoder.load_weights(path + "weights/" + experiment_with_suffix + '_decoder_model_weights.h5')
